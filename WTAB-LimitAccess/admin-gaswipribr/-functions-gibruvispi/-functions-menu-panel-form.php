<?php
//# Panel form sections
function _getSectionsLimitDevicesFields(){
    $ary_ = array(
        'Authorized Devices' => 
            array(
                'section_slug' => 'section_authorizeddevices_slug',
                'fields' => array(
                    'How many devices?' => 
                        array(
                            'field_slug' => 'field_authorizeddevices_slug',
                            'field_type' => 'text' //# It could be: text, textarea, select, checkbox
                        )
                    ,
                    'Message' => 
                        array(
                            'field_slug' => 'field_msg_authorizeddevices_slug',
...
//# Panel form register
function register_pnl_form_wawunahisp_settings() {
    $ary = _getSectionsLimitDevicesFields();
    foreach ($ary as $section_label => $section_conf){
        ...
        foreach ($fields as $field_label => $field_conf){
            if($field_label !== '-'){
                register_setting( 'wawunahisp_pnl_form_settings_page', $field_conf['field_slug'] );
            }
        }
    }
}
...
//# Panel form page
function wawunahisp_pnl_form_settings_page() {?>
    <div class="wrap">
        <h1>Panel form</h1>
    </div>
    <form class="admin_wawunahisp_pnl_form_settings_page_table" method="post" action="options.php">
        <?php settings_fields( 'wawunahisp_pnl_form_settings_page' ); ?>
        <?php do_settings_sections( 'wawunahisp_pnl_form_settings_page' ); ?>
        <table><?php
            $ary = _getSectionsLimitDevicesFields();
            foreach ($ary as $section_label => $section_conf){
                $section_slug = $section_conf['section_slug'];
